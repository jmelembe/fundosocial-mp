...
if (isset($_POST['submit'])) {
   //Capturar os dados do Formulario
	...
	if ($result) {
		header("Location: gestao_transacaoA2.php?msg=Dados registrados com Sucesso!");
	} else {
...
<body>
	...
	<form action="#" method="POST" onsubmit="return confirm('Tem certeza de que deseja prosseguir com esta operação?');">
      <hr>
		<label for="id">ID:</label>
		<input type="number" id="id" size="10" name="id" value="<?php echo $row['id'] ?>" readonly/><br>
  
		<label for="Jan">Janeiro:</label>
		<input type="text" id="Jan" size="10" name="Jan" value="<?php echo $row['Jan'] ?>" readonly/><br>
